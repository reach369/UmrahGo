...
// Helper function to switch locale while preserving the current path
export function switchLocale(currentPathname: string, currentLocale: string, targetLocale: string): string {
  // Handle case where currentPathname is null or undefined
  if (!currentPathname) return `/${targetLocale}`;
  
  // Split the path into segments
  const segments = currentPathname.split('/').filter(Boolean);
  
  // Remove all locale segments to avoid duplicates
  const pathWithoutLocales = segments.filter(segment => !routing.locales.includes(segment as any));
  
  // Create a clean path with the target locale
  return `/${targetLocale}${pathWithoutLocales.length ? '/' + pathWithoutLocales.join('/') : ''}`;
}
