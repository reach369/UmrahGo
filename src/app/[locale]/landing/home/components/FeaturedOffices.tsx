...
// Services and Types
import officeService from '@/services/officesService';
import type { Office } from '@/services/officesService';
import { getValidImageUrl } from '@/config/api.config';

// API storage base URL
const API_STORAGE_URL = process.env.NEXT_PUBLIC_API_STORAGE_URL || 'https://admin.umrahcom.net/storage/';
...
export default function FeaturedOffices() {
  ...
  useEffect(() => {
    const getFeaturedOffices = async () => {
      try {
        setLoading(true);
        const response = await officeService.fetchFeaturedOffices();
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  <Image 
                    ...
                    alt={office.office_name || 'Office Logo'}
                    fill
