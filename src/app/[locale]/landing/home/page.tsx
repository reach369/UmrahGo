  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          officesData = await landingService.searchOfficesWithFilters({ is_featured: true, per_page: 3 });
        } catch (error) {
    ...
    if (!loading && featuredOffices.length === 0) {
      const fetchOfficesDirectly = async () => {
        ...
        try {
          ...
          const response = await fetch('https://umrahgo.reach369.com/api/v1/public/offices?is_featured=true&per_page=3');
