...
function PackagesPageContent({ params }: PackagesPageProps) {
  ...
  const applyFiltersAndSort = useCallback(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(pkg => 
        ...
        pkg.office?.office_name?.toLowerCase().includes(query) ||
        pkg.start_location?.toLowerCase().includes(query) ||
        pkg.end_location?.toLowerCase().includes(query)
      );
    }
    
    // Apply accommodation filter
    if (hasAccommodation !== null) {
      filtered = filtered.filter(pkg => pkg.includes_accommodation === hasAccommodation);
    }
    ...
    if (ratingFilter !== null) {
      filtered = filtered.filter(pkg => (pkg.rating || 0) >= ratingFilter);
    }
  ...
  }, [allPackages, priceRange, durationRange, searchQuery, ratingFilter, sortBy, hasAccommodation]);
          ...
          <div className="flex flex-wrap md:flex-nowrap gap-8">
            ...
            <div className="w-full md:w-1/4 hidden md:block">
              <div className="bg-card rounded-xl p-6 border border-primary/10 shadow-md sticky top-24">
                ...
                <div className="mb-6">
                  <h4 className="font-medium mb-3">{t('packages.accommodationFilter') || 'الإقامة'}</h4>
                  <div className="space-y-2">
                    <div className="flex items-center">
                      <Checkbox
                        id="accommodation-yes"
                        checked={hasAccommodation === true}
                        onCheckedChange={() => setHasAccommodation(hasAccommodation === true ? null : true)}
                        className="data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                      />
                      <Label
                      ...
                      >
                        {t('packages.hasAccommodation') || 'تتضمن إقامة'}
                      </Label>
                    </div>
                    <div className="flex items-center">
                      <Checkbox
                        ...
                        onCheckedChange={() => setHasAccommodation(hasAccommodation === false ? null : false)}
                        className="data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                      />
                      <Label
                        ...
                        className="ml-2 text-sm font-normal cursor-pointer"
                      >
                        {t('packages.noAccommodation') || 'بدون إقامة'}
                      </Label>
                    </div>
                  </div>
                </div>
              ...
              <div className="fixed inset-0 bg-black/50 z-40 md:hidden">
                ...
                >
                  ...
                  <div className="space-y-6">
                    ...
                    {/* Accommodation Filter Mobile */}
                    <div>
                      <h4 className="font-medium mb-3">{t('packages.accommodationFilter') || 'الإقامة'}</h4>
                      <div className="space-y-2">
                        <div className="flex items-center">
                          <Checkbox
                            id="mobile-accommodation-yes"
                            checked={hasAccommodation === true}
                            onCheckedChange={() => setHasAccommodation(hasAccommodation === true ? null : true)}
                            className="data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                          />
                          <Label
                            htmlFor="mobile-accommodation-yes"
                            className="ml-2 text-sm font-normal cursor-pointer"
                          >
                            {t('packages.hasAccommodation') || 'تتضمن إقامة'}
                          </Label>
                        ...
                        <div className="flex items-center">
                          <Checkbox
                            id="mobile-accommodation-no"
                            checked={hasAccommodation === false}
                            onCheckedChange={() => setHasAccommodation(hasAccommodation === false ? null : false)}
                            className="data-[state=checked]:bg-primary data-[state=checked]:border-primary"
                          />
                          <Label
                            htmlFor="mobile-accommodation-no"
                            className="ml-2 text-sm font-normal cursor-pointer"
                          >
                            {t('packages.noAccommodation') || 'بدون إقامة'}
                          </Label>
                    ...
                    >
                      <div className="relative h-48 overflow-hidden">
                        ...
                        {pkg.includes_accommodation && (
                          <div className={`absolute top-4 ${pkg.is_featured ? 'left-4' : 'right-4'}`}>
                            <Badge className="bg-primary hover:bg-primary text-white flex items-center gap-1">
                              <BedDouble className="h-3 w-3" />
                              {t('packages.accommodation') || 'إقامة'}
                            </Badge>
                          </div>
                        )}
                          ...
                          <div className="flex items-center">
                            ...
                            <span>{pkg.duration_days} {t('packages.days') || 'أيام'}</span>
                              ...
                              <div className="flex items-center">
                                ...
                                <span className="line-clamp-1" title={pkg.start_location || '-'}>{pkg.start_location || '-'}</span>
                              </div>

                              <div className="flex items-center">
                                <BedDouble className="h-4 w-4 text-primary mr-2" />
                                <span>{pkg.includes_accommodation 
                                  ? t('packages.includesAccommodation') || 'تتضمن إقامة' 
                                  : t('packages.noAccommodationIncluded') || 'لا تتضمن إقامة'}
                                </span>
                              </div>
                            </div>
