...
import { TransportChatMessage } from './ChatMessage';
import { TransportChatInput } from './ChatInput';
import { TransportChatList } from './ChatList';
import { useTransportChatWebSocket } from '../../hooks/useChatWebSocket';
import {
  useGetChatsQuery,
...
interface TransportChatContainerProps {
  userId: string;
  userType: 'transport_operator' | 'office' | 'pilgrim';
}

export const TransportChatContainer: React.FC<TransportChatContainerProps> = ({ userId, userType }) => {
  const dispatch = useDispatch();
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const { sendWebSocketMessage } = useTransportChatWebSocket(userId);

  // Redux queries and mutations
  const { data: chats = [] } = useGetChatsQuery();
  const selectedChatId = useSelector((state: any) => state.transportChat.selectedChatId);
      ...
      <div className="flex-1 flex flex-col">
        {selectedChatId ? (
          <>
            ...
            <TransportChatInput onSendMessage={handleSendMessage} />
          </>
        ) : (
